setwd("~/Dropbox/hsf/23-ws/ewa/rmd_desc")
...
pacman::p_load(tidyverse, haven, psych, labelled, ggthemes)
...
ggplot(dflong) +
  geom_boxplot(aes(x=as_factor(id_group), y=value) )

ggplot(dflong) +
  geom_boxplot(aes(x=as_factor(id_group), y=value, color=sex, fill= as_factor(ag))) 

dsq <- dflong |> 
  filter(m=="m1") |> 
  ggplot() +
  geom_boxplot(aes(x=id_group, y=value, color=as_factor(sex), fill= as_factor(ag)))
dsq
